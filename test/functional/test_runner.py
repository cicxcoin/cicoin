...
`test/functional/test_framework/test_framework.py:CicoinTestFramework.main`.
...
def main():
    ...
    enable_cicoind = config["components"].getboolean("ENABLE_BITCOIND")

    if not enable_cicoind:
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, runs_ci, use_term_control):
    ...
    # Warn if cicoind is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "cicoind"]) is not None:
            print("%sWARNING!%s There is already a cicoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
...
class RPCCoverage():
    ...
    commands per `cicoin-cli help` (`rpc_interface.txt`).
