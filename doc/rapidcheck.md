# RapidCheck property-based testing for Cicoin Core

## Concept

Property-based testing is experimentally being added to Cicoin Core with
...
If RapidCheck is installed, Cicoin Core will automatically run the
...
3. Configure Cicoin Core with RapidCheck.

    `cd` to the directory of your local cicoin repository and run
...
4. Build Cicoin Core with RapidCheck.
    ...
    Making all in src
    [...]
    CXX      test/gen/test_cicoin-crypto_gen.o
    CXX      test/test_cicoin-key_properties.o
    ```
...
That's it! You are now running property-based tests in Cicoin Core.
