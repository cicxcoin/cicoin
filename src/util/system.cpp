...
const char * const BITCOIN_CONF_FILENAME = "cicoin.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "cicoind -foo=bar
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //cicoin-tx using stdin
        std::string val;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Cicoin";
#else
    // Unix
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Cicoin Core copyright is not removed by accident
    if (copyright_devs.find("Cicoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Cicoin Core developers";
    }
