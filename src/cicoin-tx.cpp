...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " cicoin-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  cicoin-tx [options] <hex-tx> [commands]  Update hex-encoded cicoin transaction\n" +
            "or:     cicoin-tx [options] -create [commands]   Create hex-encoded cicoin transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded cicoin transaction
